...
struct FastLinearRecurrence {
     ...
     */
    FastLinearRecurrence(vector<num_t> orig_c, num_t d) : p(move(orig_c), d, 1) {
        m = (int)p.c.size();
        powerJumps.reserve(65);
        Poly q = p;
        powerJumps.emplace_back(q);
        for (int jump_index = 1; (1 << jump_index) < m; jump_index++) {
            q = double_jump(q);
            powerJumps.emplace_back(q);
        }
    }
    ...
    /* Supported length [0, m + least_power_2(m)) or [0, sum(powerJump.jump)) or [0, 2m)
    ...
    Poly reduce(Poly p) const {
        ...
        for (int jump_index = (int)powerJumps.size() - 1; jump_index >= 0; jump_index--) {
            ...
            const auto &part = convolution(
                    vector<num_t>(coefficient.begin() + start, coefficient.end()),
                    jump.c
            );
    ...
    Poly calculate(long long jump) const {
        if (jump - powerJumps.back().jump < m) {
            for (int jump_index = (int)powerJumps.size() - 1; jump_index >= 0; jump_index--) {
                ...
                if (p.jump < jump) {
                    auto offset = static_cast<int>(jump - p.jump);
                    Poly result(vector<num_t>(p.c.size() + offset), p.d, p.jump);
